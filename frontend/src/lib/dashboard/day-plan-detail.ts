...
export interface DetailedPlanTask {
  ...
  meta?: string;
  /** Hard deadline on this plan day (e.g. due 11:59 PM that night). */
  dueThatDay?: boolean;
}
...
export function getDetailedPlanTasksForWeekday(
  mondayFirstIndex: number,
): DetailedPlanTask[] {
  return DETAILED_PLAN_BY_MONDAY_INDEX[mondayFirstIndex] ?? [];
}

/** Total tasks and count with a deadline on that calendar day (demo: `dueThatDay` flag). */
export function getPlanDayTaskStats(mondayFirstIndex: number): {
  total: number;
  dueThatDay: number;
} {
  const tasks = getDetailedPlanTasksForWeekday(mondayFirstIndex);
  return {
    total: tasks.length,
    dueThatDay: tasks.filter((t) => t.dueThatDay === true).length,
  };
}
